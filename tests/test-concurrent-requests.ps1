...
Write-Host ""
Write-Host "  C. Custom JSON Request" -ForegroundColor Magenta
Write-Host ""
Write-Host "Enter request numbers separated by commas (e.g., 1,2,3)" -ForegroundColor Yellow
Write-Host "Or enter 'C' to input a custom JSON request" -ForegroundColor Yellow
...
} elseif ($requestSelection -eq "C" -or $requestSelection -eq "c") {
    # Custom JSON request
    Write-Host ""
    Write-Host "============================================" -ForegroundColor Cyan
    Write-Host "  Custom JSON Request Input" -ForegroundColor Cyan
    Write-Host "============================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Paste your JSON request (all parameters required):" -ForegroundColor Yellow
    Write-Host "Example format:" -ForegroundColor Gray
    Write-Host '{"suburb":"North Ipswich","state":"QLD","property_types":null,"min_bedrooms":1,"max_bedrooms":null,"min_bathrooms":1,"max_bathrooms":null,"min_carspaces":1,"max_carspaces":null,"include_surrounding_suburbs":false,"post_code":"4380","region":null,"area":null,"min_land_area":null,"max_land_area":null,"home_owner_pricing":"North Ipswich"}' -ForegroundColor Gray
    Write-Host ""
    Write-Host "Paste JSON on a single line and press Enter:" -ForegroundColor Yellow
    
    # Read single-line JSON input
    $jsonInput = Read-Host
    
    # Try to parse JSON
    try {
        $customBody = $jsonInput | ConvertFrom-Json
        
        # Validate required fields
        $requiredFields = @("suburb", "state", "post_code", "home_owner_pricing")
        $missingFields = @()
        
        foreach ($field in $requiredFields) {
            if (-not $customBody.PSObject.Properties.Name.Contains($field)) {
                $missingFields += $field
            }
        }
        ...
        # Create custom request name
        $customName = "Custom: $($customBody.suburb), $($customBody.state)"
        
        # Convert JSON object to hashtable for API request
        $customBodyHash = @{
            suburb = $customBody.suburb
            state = $customBody.state
            property_types = $customBody.property_types
            min_bedrooms = $customBody.min_bedrooms
            max_bedrooms = $customBody.max_bedrooms
            min_bathrooms = $customBody.min_bathrooms
            max_bathrooms = $customBody.max_bathrooms
            min_carspaces = $customBody.min_carspaces
            max_carspaces = $customBody.max_carspaces
            include_surrounding_suburbs = $customBody.include_surrounding_suburbs
            post_code = $customBody.post_code
            region = $customBody.region
            area = $customBody.area
            min_land_area = $customBody.min_land_area
            max_land_area = $customBody.max_land_area
            home_owner_pricing = $customBody.home_owner_pricing
        }
        ...
        Write-Host "[INFO] Custom request added: $customName" -ForegroundColor Green
            ...
            foreach ($job in $jobResults) {
                try {
                    ...
                    if ($status.status -eq "completed" -and $status.dropbox_url) {
                        Write-Host "  Agent Report: $($status.filename)" -ForegroundColor Gray
                        if ($status.completed_pdf_url) {
                            Write-Host "  Completed PDF: $($status.completed_filename)" -ForegroundColor Magenta
                        }
                    }
            ...
            if ($completedCount + $failedCount -eq $jobResults.Count) {
                ...
                foreach ($job in $jobResults) {
                    ...
                    if ($status.status -eq "completed") {
                        ...
                        if ($status.completed_pdf_url) {
                            Write-Host "  Completed PDF: $($status.completed_pdf_url)" -ForegroundColor Magenta
                        }
                    } else {
                        Write-Host ""
